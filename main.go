...
func resourceManifestCreate(d *schema.ResourceData, m interface{}) error {
	...
	validate, _ := d.GetOk("validate")

	var cmd *exec.Cmd

	if isNamespace {
		cmd = kubectl(m, kubeconfig, "apply", "--validate", strconv.FormatBool(validate.(bool)), "-n", namespace.(string), "-f", "-")
	} else {
		cmd = kubectl(m, kubeconfig, "apply", "--validate", strconv.FormatBool(validate.(bool)), "-f", "-")
	}
